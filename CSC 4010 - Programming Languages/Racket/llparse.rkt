...
(define generates-epsilon?
  (lambda (w knowledge grammar)
    ...
    (cond
        ...
        ((non-terminal? w grammar) (cadr (symbol-knowledge w knowledge)))
        ((null? w) #t)
        (list? w
            (and ;"iteration" via recursion to return a single boolean
                (generates-epsilon? (car w) knowledge grammar)
...
(define first
  (lambda (w knowledge grammar)
    ...
    (cond
        ...
        ((null? w) '())
        ((list? w)
            (union ;"iteration" via recursion to return a list of firsts
                (first (car w) knowledge grammar)
...
(define get-knowledge
  (lambda (grammar)
    ...
    (let* (
            (nonterminals (non-terminals grammar))
            (rights (map (lambda (s) (right-context s grammar)) nonterminals))
          )
          (letrec ((helper (lambda (knowledge)
            (let*
                ((update_knowledge
                    (lambda (old_knowledge rules curr_right)
                        (let* (
                                (curr_first (lambda (s) (first s knowledge grammar)))
                                (curr_generates_epsilon? (lambda (s) (generates-epsilon? s knowledge grammar)))
                                (curr_follow (lambda (p) (if (curr_generates_epsilon? (cadr p))(follow (car p) knowledge) '() )))
                              )
                              (list ; augmenting old_knowledge with new information
                                    (car old_knowledge)
                                    (or
                                        (cadr old_knowledge)
                                        (eval (cons 'or (map curr_generates_epsilon? (cdr rules))))
                                    )
                                    (union
                                        (caddr old_knowledge)
                                        (apply union (map curr_first (cdr rules)))
                                    )
                                    (union
                                        (cadddr old_knowledge)
                                        (apply union (map curr_first (map cadr curr_right)))
                                        (apply union (map curr_follow curr_right))
                                    )
                              )
                 )))
                 (new_knowledge (map update_knowledge knowledge grammar rights)))
                (if (equal? new_knowledge knowledge) knowledge (begin (helper new_knowledge)))
...
(define parse-table
  (lambda (grammar)
    ...
    (let ((knowledge (get-knowledge grammar)))
        (map (lambda (rules)
            (let ((LHS (car rules)) (rights (cdr rules)))
                 (cons
                    LHS
                    (map (lambda (RHS)
                        (cons
                            (union
                                ...
                                (if (generates-epsilon? RHS knowledge grammar) (follow LHS knowledge)'())
                            ) 
                            (list RHS)
                        ))
                     rights) ;get the predict-set pair
                 ) ;attach the predict-set pair to the LHS
    )) grammar))
))
